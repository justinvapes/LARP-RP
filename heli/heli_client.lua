...
local toggle_rappel = 167 -- F6. old: 154, x -- control id to rappel out of the heli. Default: INPUT_DUCK (X)
...
local polmav2_hash = GetHashKey("polmav2")
local supervol_hash = GetHashKey("supervolito")
...
Citizen.CreateThread(function()
    ...
    Wait(0)
		...
		if playerInfo.inPolmav then
			local lPed = playerInfo.ped
			local heli = playerInfo.vehPedIsIn
			
			if IsHeliHighEnough(heli) then
				if IsControlJustPressed(0, toggle_helicam) then -- Toggle Helicam
					PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
					SendNUIMessage({toggleHelicamPanels = true, toggle = true})
					helicam = true
				end
				...
				if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
					...
					if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
						PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
						TaskRappelFromHeli(playerInfo.ped, 1)
					else
		...
		if helicam then
			...
			while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) do
				if IsControlJustPressed(0, toggle_helicam) then -- Toggle Helicam
					PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
					SendNUIMessage({toggleHelicamPanels = true, toggle = false})
		...
		if playerInfo.inPolmav and target_vehicle and not helicam and vehicle_display ~=2 then
...
function ChangeVision()
    ...
    SeethroughSetFadeStartDistance(200.0)
    SeethroughSetFadeEndDistance(250.0)
    SeethroughSetNoiseAmountMax(0.01)
...
function GetVehicleInView(cam)
	...
	local rayhandle = CastRayPointToPoint(coords, coords+(forward_vector*200.0), 10, GetVehiclePedIsIn(playerInfo.ped), 0)
...
function RenderVehicleInfo(vehicle) -- Oh my GODDD this code is horrible, like all of it
  Citizen.CreateThread(function()
        ...
        vehspeed = GetEntitySpeed(vehicle)*3.6
			...
			if (vehicle_display == 0) then
				if (vehname == "NULL") then
					SendNUIMessage({updateVehicleInfo = true, model = "N/A", speed = string.format("%s %s", math.ceil(vehspeed), speed_measure), plate = licenseplate})
				else
					SendNUIMessage({updateVehicleInfo = true, model = vehname, speed = string.format("%s %s", math.ceil(vehspeed), speed_measure), plate = licenseplate})
				end
			elseif (vehicle_display == 1) then
				if (vehname == "NULL") then
					SendNUIMessage({updateVehicleInfo = true, model = "N/A", plate = licenseplate})
				else
					SendNUIMessage({updateVehicleInfo = true, model = vehname, plate = licenseplate})
				end
			end
    end
  end)
...
Citizen.CreateThread(function()
  while true do
    playerInfo.ped = PlayerPedId()
    playerInfo.vehPedIsIn = GetVehiclePedIsIn(playerInfo.ped)
...
Citizen.CreateThread(function()
  while true do
    ...
    playerInfo.inPolmav = IsVehicleModel(playerInfo.vehPedIsIn, polmav_hash) or IsVehicleModel(playerInfo.vehPedIsIn, polmav2_hash) or IsVehicleModel(playerInfo.vehPedIsIn, supervol_hash)
  end
end)
